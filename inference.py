...
def demix_base(mix, device, models, infer_session):
    ...
    for model in models:
        ...
        while i < n_sample + pad:
            ...
            i += gen_size
        mix_waves = torch.stack(mix_waves)  # Convert the list to a single torch tensor
        mix_waves = mix_waves.to(device, dtype=torch.float32)
...
class EnsembleDemucsMDXMusicSeparationModel:
    def __init__(self, options):
        ...
        self.models = []
        self.weights_vocals = torch.tensor([10, 1, 8, 9])
        self.weights_bass = torch.tensor([19, 4, 5, 8])
        self.weights_drums = torch.tensor([18, 2, 4, 9])
        self.weights_other = torch.tensor([14, 2, 5, 10])
    ...
    def separate_music_file(
        ...
        audio = mixed_sound_array.T.unsqueeze(0).type('torch.FloatTensor').to(self.device)
        ...
        if not only_vocals:
            ...
            def process_model(i, model):
                if i == 0:
                    ...
                    overlap = overlap_large
                out = 0.5 * apply_model(model, audio, shifts=shifts, overlap=overlap)[0] \
                      + 0.5 * -apply_model(model, -audio, shifts=shifts, overlap=overlap)[0]
...
def predict_with_model(options):
    ...
    for i, input_audio in enumerate(options['input_audio']):
        ...
        for instrum in all_instrum:
            ...
            sf.write(output_folder + '/' + output_name, result[instrum].cpu().numpy(), sample_rates[instrum], subtype='FLOAT')
